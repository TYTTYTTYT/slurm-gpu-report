...
run_nodes_view() {
  ...
  # Added || true to prevent crash on timeout
  mapfile -t SINFOROWS < <(sinfo -aN -h --states=all -o "%n %G %P" || true)
  ...
  # 2) Per-node allocations (+ jobs).
  while IFS=$'\t' read -r nodelist gcount jid user part; do
    
    # Check if expansion is needed/possible
    expanded=0
    if [[ "$nodelist" == *"["*"]"* ]] && command -v scontrol >/dev/null 2>&1; then
      # FIX: scontrol might fail on weird "Reason" strings. Catch it with || echo to avoid set -e crash.
      while read -r one; do
        [[ -z "$one" ]] && continue
        # FIX: Use safe assignment `var=$((...))` instead of `(( var=... ))` which crashes if result is 0
      ...
      done < <(scontrol show hostnames "$nodelist" 2>/dev/null || echo "$nodelist")
      expanded=1
    fi

    # Fallback (no brackets or scontrol failed)
    if [[ $expanded -eq 0 ]]; then
      node="$nodelist"
      [[ -z "$node" || "$node" == "(Priority)" || "$node" == "n/a" ]] && continue
      
      # FIX: Safe assignment here too
    ...
    # FIX: Added || true to prevent crash on empty/failed squeue
    (squeue -h -o "%R|%b|%i|%u|%P" || true) \
    | awk -F"|" 'BEGIN{OFS="\t"}{
  ...
  build_tsv() {
    ...
    for node in "${ORDERED_NODES[@]}"; do
      ...
      # FIX: "(( idle < 0 ))" returns exit code 1 if false, crashing the script.
      # We change it to a standard 'if' block which is safe.
      idle=$(( total - alloc ))
      if (( idle < 0 )); then idle=0; fi
...
run_jobs_view() {
  build_tsv() {
    ...
    (squeue -h -t all -o "%i|%u|%P|%j|%t|%M|%D|%R|%b" || true) \
    | awk -F"|" 'BEGIN{OFS="\t"}{
...
run_users_view() {
  ...
  mapfile -t ROWS < <(squeue -h -t all -o "%u|%i|%P|%R|%b" || true)
  for line in "${ROWS[@]}"; do
    [[ -z "$line" ]] && continue
    user="${line%%|*}"; rest="${line#*|}"
    jid="${rest%%|*}"; rest="${rest#*|}"
    ...
    # FIX: Safe assignment. (( var = ... )) crashes if result is 0.
  ...
  build_tsv() {
    ...
    for user in "${!JCOUNT[@]}"; do
      jobs="${JCOUNT[$user]}"
      gpus="${GPUS[$user]:-0}"
      parts="${USERS_PARTS[$user]:--}"
      nodes_raw="${USERS_NODES[$user]:--}"
